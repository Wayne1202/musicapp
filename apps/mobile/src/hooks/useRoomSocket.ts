 ...
 *
 * Also reconnects on app foreground: unlike a browser tab, a backgrounded RN app can have its
 * JS timers frozen and its socket transport dropped by the OS, so there's no guarantee the
 * normal "disconnect"/"connect" socket.io lifecycle fires on its own when the app resumes.
 */
export function useRoomSocket(roomId: string | null, sessionId: string | null) {
  ...
  useEffect(() => {
    ...
    const onAppStateChange = (next: AppStateStatus) => {
      if (next === "active" && !socket.connected) {
        socket.connect();
      }
    };
