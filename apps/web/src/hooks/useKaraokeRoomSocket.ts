...
/**
 * Mirrors useRoomSocket.ts's shape (reducer-based room state sync) but for the karaoke room, and
 * mirrors apps/mobile's useKaraokeRoomSocket.ts's event wiring exactly (same backend contract).
 * WebRTC signaling events are NOT folded into the reducer's state — they're handed off live to
 * `signaling` callbacks instead, since they drive imperative RTCPeerConnection calls
 * (useKaraokeWebRTC.ts), not React state.
 */
